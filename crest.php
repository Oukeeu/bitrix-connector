<?php
	...
	class CRest
	{
		...
		{
			$return = [];
			if(file_exists(__DIR__ . '/tmp/' . SERVER_PATH . '/settings.json'))
			{
				$return = static::expandData(file_get_contents(__DIR__ . '/tmp/' . SERVER_PATH . '/settings.json'));
		...
		{
			return  (boolean)file_put_contents(__DIR__ . '/tmp/' . SERVER_PATH . '/settings.json', static::wrapData($arSettings));
		}
		...
		public static function setLog($arData, $type = '')
		{
			...
			if(!defined("C_REST_BLOCK_LOG") || C_REST_BLOCK_LOG !== true)
			{
				...
				{
					$path = __DIR__ . '/tmp/' . SERVER_PATH . '/logs/';
				}
		...
		public static function checkServer($print = true)
		{
			...
			//creat setting file
			file_put_contents(__DIR__ . '/tmp/' . SERVER_PATH . '/settings_check.json', static::wrapData(['test'=>'data']));
			if(!file_exists(__DIR__ . '/tmp/' . SERVER_PATH . '/settings_check.json'))
			{
				$return['setting_creat_error'] = 'Check permission! Recommended: folders: 775, files: 664';
			}
			unlink(__DIR__ . '/tmp/' . SERVER_PATH . '/settings_check.json');
			//creat logs folder and files
			$path = __DIR__ . '/tmp/' . SERVER_PATH . '/logs/' . date("Y-m-d/H") . 'crest.php/';
